...
(function ($) {
	...
	$.extend(CkEditable.prototype, {
		render: function () {
			...
			CKEDITOR.config.protectedSource.push(/<(html)[^>]*>.*<\/script>/ig);
			CKEDITOR.config.protectedSource.push(/<(body)[^>]*>.*<\/script>/ig);
			CKEDITOR.config.protectedSource.push(/<(head)[^>]*>.*<\/script>/ig);
